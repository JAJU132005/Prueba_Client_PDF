/* <panda-art> — biblioteca de ilustraciones SVG "Diario del Panda".
   Trazo simple tipo marcador, relleno plano. Colores desde los tokens CSS
   (var(--ink), var(--card), …) para funcionar en modo cuaderno y pizarra.
   Uso: <panda-art kind="unir" style="width:320px"></panda-art>
   kinds: 15 slugs de herramienta · pose-ligera/media/pesada · portada ·
          comic1/2/3 · trituradora · nube                                  */
(function () {
  if (customElements.get('panda-art')) return;

  var INK = 'var(--ink,#2d2a26)';
  var PAPER = 'var(--card,#fffdf6)';
  var SOFT = 'var(--ink-soft,#8a857b)';
  var RED = 'var(--mk-red,#d84438)';
  var ORANGE = 'var(--mk-orange,#d0821a)';
  var GREEN = '#8fd14f', GREEND = '#4c7a2c';
  var HAND = "font-family:'Gochi Hand',cursive";

  function at(x, y, s, inner) { return '<g transform="translate(' + x + ' ' + y + ') scale(' + (s || 1) + ')">' + inner + '</g>'; }
  ...
  function head(o) {
    ...
    var eyes;
    if (o.sleep) {
      eyes = '<path d="M-9 0 q2.5 3 5 0 M4 0 q2.5 3 5 0" fill="none" stroke="#fff" stroke-width="1.8" stroke-linecap="round"/>';
    } else if (o.shades) {
      eyes = '<rect x="-12" y="-4" width="10.5" height="7.5" rx="3" fill="#15130f"/><rect x="1.5" y="-4" width="10.5" height="7.5" rx="3" fill="#15130f"/><line x1="-1.5" y1="-1" x2="1.5" y2="-1" stroke="#15130f" stroke-width="2"/>';
    } else {
      var px = o.look === 'left' ? -1.2 : o.look === 'right' ? 1.2 : 0;
      var py = o.look === 'up' ? -1 : 0.4;
      eyes = '<circle cx="-6.5" cy="0" r="2.9" fill="#fff"/><circle cx="6.5" cy="0" r="2.9" fill="#fff"/>' +
        '<circle cx="' + (-6.5 + px) + '" cy="' + py + '" r="1.6" fill="' + INK + '"/>' +
        '<circle cx="' + (6.5 + px) + '" cy="' + py + '" r="1.6" fill="' + INK + '"/>';
    }
    var mouth = o.mouth || 'M-4 9 Q0 12.5 4 9';
    var extras = '';
    if (o.tongue) extras += '<path d="M1 10.5 q2.5 4.5 4.5 1.5 q.8 -1.8 -1.2 -2.6z" fill="#e58a8a" stroke="' + INK + '" stroke-width="1"/>';
    if (o.beret) extras += '<path d="M-14 -13 Q0 -24 14 -13 Q0 -17 -14 -13z" fill="#3b5da8" stroke="' + INK + '" stroke-width="1.6"/><circle cx="0" cy="-19" r="2" fill="#3b5da8" stroke="' + INK + '" stroke-width="1.2"/>';
    if (o.sherlock) extras += '<path d="M-15 -12 L0 -26 L15 -12 Q0 -18 -15 -12z" fill="#e8dcc0" stroke="' + INK + '" stroke-width="1.6"/><line x1="-15" y1="-12" x2="15" y2="-12" stroke="' + INK + '" stroke-width="1.6"/>';
    if (o.cap) extras += '<path d="M-13 -13 Q0 -21 13 -13 L16 -10 L10 -12 Q0 -18 -13 -13z" fill="#d8b23a" stroke="' + INK + '" stroke-width="1.4"/>';
    return '<g transform="rotate(' + (o.tilt || 0) + ')">' +
  ...
  function panda(x, y, s, ho, armL, armR) {
    var inner =
      limb(-7, 6, -10, 20) + limb(7, 6, 10, 20) +
      body() +
      (armL || limb(-11, -3, -19, 5)) + (armR || limb(11, -3, 19, 5)) +
      at(0, -23, 1, head(ho || {}));
  ...
  function paper(x, y, w, h, rot, lines, extra) {
    var g = '<rect width="' + w + '" height="' + h + '" rx="3" fill="#fff" stroke="' + INK + '" stroke-width="2.4"/>';
    var n = lines == null ? 4 : lines;
    for (var i = 1; i <= n; i++) {
  ...
  function bamboo(x, y, h) {
    var g = '<rect x="-3.5" y="0" width="7" height="' + h + '" rx="3" fill="' + GREEN + '" stroke="' + GREEND + '" stroke-width="1.6"/>';
    for (var i = 1; i < h / 16; i++) g += '<line x1="-3.5" y1="' + i * 16 + '" x2="3.5" y2="' + i * 16 + '" stroke="' + GREEND + '" stroke-width="1.6"/>';
  ...
  var ART = {};
  function def(k, vb, label, fn) { ART[k] = { vb: vb, label: label, svg: fn }; }

  /* ---------- POSES DE NIVEL (badges) ---------- */
  def('pose-ligera', '0 0 96 66', 'Panda relajado en silla de playa con limonada de bambú', function () {
    return limb(20, 58, 62, 30, 4) + limb(20, 30, 62, 58, 4) + limb(14, 40, 70, 40, 4) +
      at(40, 34, 0.85,
        '<ellipse cx="0" cy="4" rx="15" ry="12" fill="' + PAPER + '" stroke="' + INK + '" stroke-width="2.6" transform="rotate(-18)"/>' +
        limb(-10, 12, -20, 20) + limb(2, 14, -6, 24) + limb(10, 0, 24, 2) +
        at(-8, -17, 1, head({ tilt: -14, mouth: 'M-4 9 Q0 12 4 9' }))) +
      at(74, 44, 1, '<rect x="-5" y="-8" width="10" height="16" rx="2" fill="#fdf3b8" stroke="' + INK + '" stroke-width="1.8"/><line x1="1" y1="-8" x2="6" y2="-20" stroke="' + GREEND + '" stroke-width="2.4" stroke-linecap="round"/>') +
      '<text x="6" y="14" style="' + HAND + ';font-size:11px" fill="' + SOFT + '">ahhh…</text>';
  });
  def('pose-media', '0 0 96 66', 'Panda trotando con una gotita de sudor', function () {
    return '<path d="M8 30 h14 M6 40 h12" stroke="' + SOFT + '" stroke-width="2.4" stroke-linecap="round"/>' +
      at(52, 38, 0.85,
        limb(-6, 8, -16, 18) + limb(6, 8, 16, 14) +
        '<ellipse cx="0" cy="0" rx="15" ry="12.5" fill="' + PAPER + '" stroke="' + INK + '" stroke-width="2.6" transform="rotate(8)"/>' +
        limb(-10, -6, -22, -2) + limb(10, -4, 20, -12) +
        at(8, -22, 1, head({ tilt: 10, tongue: true, mouth: 'M-4 9 Q0 11 4 9' }))) +
      sweat(76, 10);
  });
  def('pose-pesada', '0 0 96 66', 'Panda levantando una pesa gigante, con vapor en las orejas', function () {
    return '<path d="M28 6 q-3 -4 0 -7 M68 6 q3 -4 0 -7" stroke="' + SOFT + '" stroke-width="2" fill="none" stroke-linecap="round"/>' +
      limb(22, 14, 74, 14, 5) +
      '<circle cx="20" cy="14" r="9" fill="' + INK + '"/><circle cx="76" cy="14" r="9" fill="' + INK + '"/>' +
      at(48, 44, 0.85,
        limb(-7, 8, -10, 20) + limb(7, 8, 10, 20) + body() +
        limb(-10, -6, -16, -26) + limb(10, -6, 16, -26) +
        at(0, -23, 1, head({ mouth: 'M-4 11 Q0 8.5 4 11' }))) +
      '<path d="M14 34 l-4 3 M82 34 l4 3" stroke="' + SOFT + '" stroke-width="2" stroke-linecap="round"/>';
  });

  /* ---------- PORTADA / HERO ---------- */
  def('portada', '0 0 360 240', 'Portada del diario: cuaderno con candado y panda saludando', function () {
    var spiral = '';
    for (var i = 0; i < 8; i++) spiral += '<circle cx="78" cy="' + (48 + i * 22) + '" r="5" fill="none" stroke="' + INK + '" stroke-width="2.4"/>';
    return '<rect x="70" y="30" width="220" height="185" rx="10" fill="' + PAPER + '" stroke="' + INK + '" stroke-width="3.5"/>' +
      '<line x1="92" y1="30" x2="92" y2="215" stroke="' + INK + '" stroke-width="2" stroke-dasharray="1 6" stroke-linecap="round"/>' + spiral +
      tape(252, 26, 10) +
  ...
  def('nube', '0 0 200 110', 'Nube tachada: aquí no hay nube', function () {
    return '<g fill="' + PAPER + '" stroke="' + INK + '" stroke-width="3">' +
  ...
  /* ---------- CÓMIC ---------- */
  def('comic1', '0 0 300 150', 'Viñeta 1: tu archivo entra a tu navegador por una puerta', function () {
    return '<rect x="176" y="24" width="86" height="112" rx="4" fill="none" stroke="' + INK + '" stroke-width="3.4"/>' +
  ...
  def('comic2', '0 0 300 150', 'Viñeta 2: se procesa dentro de una caja fuerte, con el panda de guardia', function () {
    return '<rect x="88" y="38" width="124" height="100" rx="8" fill="' + PAPER + '" stroke="' + INK + '" stroke-width="3.6"/>' +
  ...
  def('comic3', '0 0 300 150', 'Viñeta 3: el archivo sale por la misma puerta, sellado. Sin servidores.', function () {
    return '<rect x="38" y="24" width="86" height="112" rx="4" fill="none" stroke="' + INK + '" stroke-width="3.4"/>' +
  ...
  /* ---------- TRITURADORA ---------- */
  def('trituradora', '0 0 300 130', 'Trituradora feliz convirtiendo los datos temporales en confeti', function () {
    var conf = '';
    var cs = [GREEN, ORANGE, RED, GREEN, ORANGE, RED, GREEN];
    for (var i = 0; i < 7; i++) {
      conf += '<rect x="' + (108 + i * 14) + '" y="' + (96 + (i % 3) * 9) + '" width="7" height="7" fill="' + (typeof cs[i] === 'string' ? cs[i] : GREEN) + '" transform="rotate(' + (i * 31 % 60 - 30) + ' ' + (110 + i * 14) + ' ' + (98 + (i % 3) * 9) + ')"/>';
    }
    return paper(128, 6, 46, 34, -5, 2) +
  ...
  def('unir', '0 0 320 170', 'La grapadora gigante: el panda grapa tus hojas', function () {
    return paper(36, 96, 64, 46, -3, 2) + paper(42, 84, 64, 46, 2, 2) + paper(48, 72, 64, 46, -1, 2) +
      at(180, 118, 1,
  ...
  def('dividir', '0 0 320 170', 'Tijeras y línea punteada entre rangos', function () {
    return paper(52, 24, 92, 122, -2, 5) +
  ...
  def('rotar', '0 0 320 170', 'La perilla de caja fuerte escolar y el panda torciendo el cuello', function () {
    var ticks = '';
    [[0, '90\u00b0'], [90, '180\u00b0'], [180, '270\u00b0']].forEach(function (t, i) {
      var a = (-90 + t[0]) * Math.PI / 180;
      ticks += '<text x="' + (108 + Math.cos(a) * 52) + '" y="' + (88 + Math.sin(a) * 52 + 5) + '" text-anchor="middle" style="' + HAND + ';font-size:14px" fill="' + SOFT + '">' + t[1] + '</text>';
    });
    return '<circle cx="108" cy="88" r="36" fill="' + PAPER + '" stroke="' + INK + '" stroke-width="3.4"/>' +
      ...
      '<circle cx="108" cy="88" r="5" fill="' + INK + '"/>' + ticks +
  ...
  def('organizar', '0 0 320 170', 'El tablón de corcho con polaroids y chinchetas', function () {
    function pola(x, y, rot, cross) {
      return '<g transform="translate(' + x + ' ' + y + ') rotate(' + rot + ')">' +
        ...
        (cross ? '<line x1="4" y1="6" x2="42" y2="38" stroke="' + RED + '" stroke-width="4" stroke-linecap="round"/><line x1="42" y1="6" x2="4" y2="38" stroke="' + RED + '" stroke-width="4" stroke-linecap="round"/>' : '') + '</g>';
    }
    return '<rect x="24" y="20" width="188" height="128" rx="6" fill="#e6c48f" stroke="' + INK + '" stroke-width="3.2"/>' +
      '<rect x="32" y="28" width="172" height="112" rx="4" fill="none" stroke="rgba(0,0,0,.18)" stroke-width="2"/>' +
      pola(46, 44, -5, false) + pola(102, 40, 4, false) + pola(154, 48, -3, true) +
  ...
  def('pdf-a-imagenes', '0 0 320 170', 'El panda fotógrafo con cámara instantánea', function () {
    return '<path d="M96 34 l10 -12 M88 40 l-14 -8 M106 44 l6 -14" stroke="' + ORANGE + '" stroke-width="3" stroke-linecap="round"/>' +
      at(150, 78, 1,
  ...
  def('imagenes-a-pdf', '0 0 320 170', 'Álbum de recortes con fotos pegadas con cinta', function () {
    return '<rect x="34" y="28" width="196" height="122" rx="8" fill="' + PAPER + '" stroke="' + INK + '" stroke-width="3.4"/>' +
      ...
      tape(144, 50, -10) +
      at(258, 52, 1, '<path d="M0 0 l-16 -10 v20z M0 0 l16 -10 v20z" fill="' + RED + '" stroke="' + INK + '" stroke-width="2"/><circle cx="0" cy="0" r="4.5" fill="' + RED + '" stroke="' + INK + '" stroke-width="2"/>') +
      panda(272, 118, 0.95, { look: 'left' }, limb(-11, -3, -24, -14), null);
  });
  def('numeros', '0 0 320 170', 'El sello numerador de oficina antigua', function () {
    return paper(48, 30, 104, 118, -1, 5,
  ...
  def('marca-de-agua', '0 0 320 170', 'El rodillo de pintura fantasma', function () {
    return paper(44, 24, 110, 126, -1, 5,
  ...
  def('comprimir', '0 0 320 170', 'La prensa panda: sentado encima de la pila', function () {
    var stack = '';
    for (var i = 0; i < 5; i++) stack += '<rect x="' + (96 - i) + '" y="' + (118 - i * 9) + '" width="' + (128 + i * 2) + '" height="8" rx="3" fill="#fff" stroke="' + INK + '" stroke-width="2"/>';
    return stack +
      '<path d="M84 128 l-8 6 M244 128 l8 6 M88 108 l-9 2 M240 108 l9 2" stroke="' + SOFT + '" stroke-width="2.4" stroke-linecap="round"/>' +
      at(160, 64, 1.05,
        limb(-9, 6, -20, 12) + limb(9, 6, 20, 12) + body() +
        limb(-11, -4, -22, 4) + limb(11, -4, 22, 4) +
        at(0, -23, 1, head({ mouth: 'M-4 10 Q0 8 4 10' }))) +
      sweat(196, 30) + ono(232, 62, '\u00a1CRONCH!', ORANGE, 20, -8) +
      at(52, 118, 1, '<path d="M0 20 l7 -20 M0 20 l-7 -20 M0 20 l0 -22" stroke="' + SOFT + '" stroke-width="2" stroke-linecap="round"/>');
  });
  def('proteger', '0 0 320 170', 'El candado del diario', function () {
    return '<rect x="52" y="30" width="150" height="116" rx="10" fill="' + PAPER + '" stroke="' + INK + '" stroke-width="3.4"/>' +
  ...
  def('firmar', '0 0 320 170', 'El autógrafo con pluma fuente gigante', function () {
    return paper(46, 34, 130, 104, -1, 3,
  ...
  def('rellenar', '0 0 320 170', 'El examen escolar con espacios en blanco', function () {
    return paper(48, 22, 132, 130, -1, 0,
      ...
      at(212, 90, 1, '<g transform="rotate(-42)"><rect x="-5" y="-30" width="10" height="44" rx="2" fill="#f2d24c" stroke="' + INK + '" stroke-width="2.2"/><path d="M-5 14 L0 26 L5 14z" fill="#e8c9a0" stroke="' + INK + '" stroke-width="2"/><rect x="-5" y="-36" width="10" height="7" rx="2" fill="#e58a8a" stroke="' + INK + '" stroke-width="2"/></g>') +
  ...
  def('redactar', '0 0 320 170', 'Expediente clasificado: panda agente secreto', function () {
    return paper(52, 24, 124, 126, -1, 0,
  ...
  def('editar', '0 0 320 170', 'El estuche completo: útiles escolares y panda artista', function () {
    function util(x, c, tip) {
      return '<g transform="translate(' + x + ' 0) rotate(-6)"><rect x="-4" y="-26" width="8" height="40" rx="3" fill="' + c + '" stroke="' + INK + '" stroke-width="2"/>' +
        (tip ? '<path d="M-4 14 L0 24 L4 14z" fill="#e8c9a0" stroke="' + INK + '" stroke-width="1.8"/>' : '<rect x="-4" y="14" width="8" height="8" rx="2" fill="' + INK + '"/>') + '</g>';
    }
    return '<rect x="40" y="96" width="150" height="46" rx="8" fill="#b86a4a" stroke="' + INK + '" stroke-width="3"/>' +
  ...
  def('ocr', '0 0 320 170', 'El panda detective con lupa gigante', function () {
    return paper(44, 24, 120, 126, -1, 0,
  ...
  class PandaArt extends HTMLElement {
    static get observedAttributes() { return ['kind', 'label']; }
    connectedCallback() { this.render(); }
    attributeChangedCallback() { if (this.isConnected) this.render(); }
    render() {
      var k = this.getAttribute('kind') || 'pose-ligera';
      var d = ART[k] || ART['pose-ligera'];
      this.style.display = 'block';
      this.style.maxWidth = '100%';
      var label = this.getAttribute('label') || d.label;
      this.innerHTML = '<svg viewBox="' + d.vb + '" role="img" aria-label="' + label.replace(/"/g, '&quot;') + '" style="display:block;width:100%;height:auto">' + d.svg() + '</svg>';
    }
  }
  customElements.define('panda-art', PandaArt);
})();
